...
function fromPage(page) {
    ...
    var startedDateTime = new Date(wallTime * 1000);
    ...
    for (var requestId in page.objects) {
        ...
        if (((object.requestMessage.request.url == "http://jezebel.com/") || (ur.indexOf("ads?gdfp") != -1)) && check == false ) {
            if(ur.indexOf("ads?gdfp") != -1){
                check = true;
            }
            entries.push({
                'pageref': page.id.toString(),
                'startedDateTime': object.requestMessage.wallTime,
